...
########################################################################
    ...
    async def corp_analysis(self, task_id, room_id, record, CONF_BASE, screen, record_player, record_dao_poker, scope, filename, inx):                      # 对截图图片裁剪并分析获取牌面信息
            ...
            for player in CORP_PLAYER:
                ...
                for dao_poker in CORP_POKER:
                    ...
                    for inx, dk in enumerate(CONF_BASE[dao_poker]):  # 对每个位置进行图像分割， 并保存进相应的文件夹
                        ...
                        tasks.append(self.corp_analysis(task_id, room_id, record, CONF_BASE[dao_poker], screen, POKER_RELA[player],
                                               POKER_RELA[dao_poker], dk, filename, inx))
                        # await asyncio.create_task(self.corp_analysis(task_id, room_id, record, CONF_BASE[dao_poker], screen, POKER_RELA[player],
                        #                        POKER_RELA[dao_poker], dk, filename, inx))
